...
interface ExtendedHttpClientContract {
  ...
  fun shouldInvalidateWhenUnsafe(method: String) {
    ...
    sequenceOf(get(urlPattern), get(urlEqualTo(locationPath)), get(urlEqualTo(contentLocationPath)))
      .forEachIndexed { i, mappingBuilder ->
        stubFor(
          mappingBuilder
            .willReturn(
              ok()
    ...
    sequenceOf(post(urlPattern), put(urlPattern), delete(urlPattern)).forEachIndexed { i, mappingBuilder ->
      val response = when (i) {
        2 -> status(500)
        else -> ok()
      }
    ...
    val await = await().atMost(Duration.ofMillis(1000))
